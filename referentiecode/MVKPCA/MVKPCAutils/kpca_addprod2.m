function [eigval, eigvec, scores, omega] = kpca_addprod2(Xtrain, kernel_type, kernel_pars, eta ,Xt,etype,nb,rho2,rescaling,v_test)
...
if (strcmpi(etype,'svd') || strcmpi(etype,'eig') || strcmpi(etype,'eigs')),
    ...
    MM=cell(1,V);
    omegas=cell(1,V);
    omega_prod=ones(nb_data);
    omega_add=zeros(nb_data);
    for v=1:V
        ...
        omega_prod=omega_prod.*omega_tmp; 
        omega_add=omega_add+omega_tmp;
    end
    omega=(1-rho2)*omega_add+rho2*omega_prod;
...
if exist('Xt','var') && ~isempty(Xt),
    if iscell(Xt) && length(Xt) == V
        nt=size(Xt{1},1);
        omega_t_prod=ones(nb_data,nt);
        omega_t_add=zeros(nb_data,nt);
        for v=1:V
            ...
            omega_t_prod=omega_t_prod.*omega_tmp;
            omega_t_add=omega_t_add+omega_tmp;
        end
        omega_t=rho2*omega_t_prod + (1-rho2)*omega_t_add;
